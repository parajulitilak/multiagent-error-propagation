...
\begin{itemize}
    \item \textbf{RQ1 (Propagation and Severity):} How often do injected
    faults survive to the final answer, and how does error severity
    (mild/moderate/severe) change the survival rate? Do verifiers only
    catch fatal errors, or are they sensitive to subtle slips?
    \item \textbf{RQ2 (Verifier Strategy):} How does verifier prompt
    strategy (direct vs chain-of-thought vs rubric) affect catch and
    correction rates across different error types and severities?
    \item \textbf{RQ3 (The Map):} How do injection stage (Decomposer vs
    Solver) and the presence of a verifier (ablation) interact with error
    type, severity, and verifier strategy to determine catch rates? A full
    map is produced identifying which verifier catches which error, where
    it was injected, how severe it was, and isolating the causal
    contribution of the verification stage.
\end{itemize}
